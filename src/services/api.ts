...
export async function checkoutCart(user: User | null, paymentMethod: PaymentMethod) { return request<OrderResponse>('/orders/checkout', { method: 'POST', body: JSON.stringify({ customer: { nome: user?.nome || 'Cliente App Art3D', email: user?.email || 'Não informado' }, payment: { method: paymentMethod, details: paymentMethod === 'Pix' ? 'QR Code Pix simulado gerado pelo backend.' : 'Pagamento com cartão simulado pelo backend.' } }) }); }
